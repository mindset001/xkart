...
const VipModal: FC<ModalProps> = ({ onCancel, ...props }) => {
  const [quantity, setQuantity] = useState(1);
  const pricePerTicket = 25000; // Price for one ticket
  ...
  const handlePay = () => {
    ...
    const url = `/payment?ticketType=VIP%20Ticket&quantity=${quantity}&totalPrice=${totalPrice}`;
    router.push(url);
  };
  ...
  return (
    <Modal
      ...
      styles={{ content: { background: 'rgba(255, 255, 255, .95)' } }}
      ...
      <Card bordered={false} style={{ boxShadow: 'none' }}>
        <main>
          <div className="flex justify-end">
            <button onClick={onCancel} className="font-[600] text-[16px] mb-[-20px] cursor-pointer">x</button>
          </div>
          <div className="flex">
            <div className="text-center">
              <h1 className="text-[#101828] text-[24px] font-[600]">Purchase Ticket for 4th Race</h1>
              <p className="text-[#EF3133] text-[18px] font-[600] my-[2px]">VIP Ticket</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <button
              className="w-[144px] rounded-br-[16px] rounded-tl-[16px] bg-[#EF3133] text-white text-[16px] font-semibold py-2 px-4 rounded-[4px]"
              onClick={handlePay}
            >
